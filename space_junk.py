...
sat_speed = 3
debris_speed = 5
laser_speed = -7  # moving LEFT, so negative x direction
...
def updateLasers():
    ...
    for laser in lasers:
        ...
        if satellite.colliderect(laser) == 1:
            ...
            clock.schedule(remove_explosion, 0.5)
            score += -5
        ...
        if debris.colliderect(laser) == 1:
            ...
            sounds.explosion.play()
            clock.schedule(remove_debris_explosion, 0.5)
            score += 10
            sounds.explosion.play()
